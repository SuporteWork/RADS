...
git clone https://github.com/SuporteWork/RADS.git /root/RADSPatch
...
if echo "$UPDATE_OUTPUT" | grep -q '^[[:alnum:]]'; then
  ...
  echo ${YELLOW}"dnf -y install wget && cd /root &&  bash <(wget -qO- https://raw.githubusercontent.com/SuporteWork/RADS/main/patch.sh)"${TEXTRESET}
  read -p "Press Enter"
  exit
else
